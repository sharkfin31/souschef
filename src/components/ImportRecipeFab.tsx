...
const METHODS: { id: ImportMethod; label: string; icon: ReactNode }[] = [
  { id: 'link', label: 'Link', icon: <LinkIcon className="size-[1.25rem]" strokeWidth={1.75} /> },
  { id: 'image', label: 'Images', icon: <ImageIcon className="size-[1.25rem]" strokeWidth={1.75} /> },
  { id: 'pdf', label: 'PDF', icon: <FileText className="size-[1.25rem]" strokeWidth={1.75} /> },
  { id: 'text', label: 'Text', icon: <PenLine className="size-[1.25rem]" strokeWidth={1.75} /> },
];

/** Long enough for staggered vertical exit (delay + animation). */
const EXIT_MS = 420;

/** Inner circle: colors + entrance animation (transform is owned by keyframes, so hover-scale lives on outer wrapper). */
const IMPORT_OPTION_CIRCLE =
  'inline-flex size-11 shrink-0 items-center justify-center rounded-full border border-border bg-background text-muted-foreground shadow-sm transition-[box-shadow,border-color,background-color,color] duration-200 ease-out';
            ...
            >
              {METHODS.map((m, index) => {
                ...
                return (
                  <div
                    key={m.id}
                    className="inline-flex origin-center transition-transform duration-200 ease-out hover:z-10 hover:scale-[1.1]"
                  >
                    <TooltipTrigger label={m.label} placement="cursor-left">
                      <button
                        type="button"
                        role="menuitem"
                        aria-label={m.label}
